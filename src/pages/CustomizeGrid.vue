            ...
            <div class="row q-mt-lg q-mb-lg relative-position"
                ...
                <div class="absolute fit column">
                    <div class="col row" v-for="i in grid.rowCount" :key="i">
                        <div class="col" style="border: 1px solid lightgray;padding: 5px;" v-for="k in grid.colCount"
                            :key="k" />
                    </div>
                </div>
                <grid-layout class="fit" v-model:layout="grid.layout" :col-num="grid.colCount" :maxRows="grid.rowCount"
                    ...
                    <grid-item v-for="(item, index) in grid.layout" :x="item.x" :y="item.y" :w="item.w" :h="item.h"
                        :i="item.i" :key="item.i">
                        <div class="fit flex flex-center">
                            <div class="bg-grey-3 rounded-borders shadow-2 flex flex-center"
                                ...
                                <q-btn @click="removeGrid(item.i)" size="sm" flat round icon="close"
